...
class VaultScreen(Screen):
    ...
    def view_file(self, filename):
        ...
        if ext in IMAGE_EXTS:
            ...
            self._open_video_externally(path)
        else:
    ...
    def _open_video_externally(self, path):
        """Hand the video to Android's own video player via an Intent.

        Kivy's VideoPlayer needs ffpyplayer to decode anything on Android,
        and ffpyplayer currently can't be built (see buildozer.spec). This
        sidesteps that entirely: instead of decoding video ourselves, we
        get a content:// URI for the file from our FileProvider (declared
        via android.res_xml / android.extra_manifest_application_arguments
        in buildozer.spec) and fire ACTION_VIEW, exactly as if the user
        had tapped this video anywhere else on their phone.
        """
        if platform != 'android':
            self._show_message('Video preview only works in the installed Android app.')
            return
        ...
        try:
            ...
            content_uri = FileProviderCls.getUriForFile(
                activity, authority, JavaFile(str(path))
            )
            ...
            Logger.error('VaultApp: could not open video externally: %s', exc)
            self._show_message("Couldn't find an app to play this video.")
